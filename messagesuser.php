...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['message']) && isset($_POST['seller_id'])) {
            ...
            if ($stmt->rowCount() > 0) {
                echo "Message sent successfully to seller ID: $sellerId.";
            } else {
...
try {
    $stmt = $pdo->prepare("SELECT id, username FROM sellers");
    $stmt->execute();
    $sellers = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Error fetching sellers: " . $e->getMessage();
}

try {
    $stmt = $pdo->prepare("SELECT id, seller_id, message FROM seller_messages WHERE user_id = ?");
    $stmt->execute([$userId]);
...
try {
    ...
    $uniqueSellers = array_unique(array_column($sentMessages, 'seller_id'));
} catch (PDOException $e) {
    echo "Error fetching sent messages: " . $e->getMessage();
}
try {
    // Fetch all messages sent by the current user
    $stmtSent = $pdo->prepare("SELECT id, seller_id, message FROM seller_messages WHERE user_id = ?");
    $stmtSent->execute([$userId]);
    $sentMessages = $stmtSent->fetchAll(PDO::FETCH_ASSOC);

    // Fetch all messages received by the current user from sellers
    $stmtReceived = $pdo->prepare("SELECT id, user_id, message FROM seller_messages WHERE seller_id = ?");
    $stmtReceived->execute([$userId]);
    $receivedMessages = $stmtReceived->fetchAll(PDO::FETCH_ASSOC);

    // Fetch unique seller IDs from received messages
    $uniqueSellersReceived = array_unique(array_column($receivedMessages, 'user_id'));
} catch (PDOException $e) {
...
<header>
    ...
    <div class="container">
        ...
        <div class="account-nav">
            <ul>
                <li><a href="My-Accountuser.php"><i class="fa-regular fa-user"></i>  My Account</a></li>
                <li><a href="Notifications.php"><i class="fa-regular fa-bell"></i>  Notification</a></li>
                <li style="background: #c0c0c0;"><a href="messagesuser.php"><i class="fa-regular fa-message"></i>  Messages</a></li>
    ...
    <form method="get">
        <select name="seller_filter">
            ...
            <?php foreach ($uniqueSellers as $sellerId) : ?>
    ...
    <ul>
        ...
        foreach ($sentMessages as $message) {
            if (isset($_GET['seller_filter']) && $_GET['seller_filter'] !== '' && $message['seller_id'] != $_GET['seller_filter']) {
                continue;
            }
        ?>
            <li>
                <strong>To Seller ID <?= $message['seller_id'] ?>:</strong>
                <?= $message['message'] ?>
            </li>
        <?php } ?>
    </ul>
...
<form method="get">
    <select name="sender_filter">
        ...
        <?php foreach ($uniqueSellersReceived as $senderId) : ?>
...
<ul>
    ...
    <?php foreach ($receivedMessages as $message) : ?>
        <?php 
            ...
            if (isset($_GET['sender_filter']) && $_GET['sender_filter'] !== '' && $message['user_id'] != $_GET['sender_filter']) {
                continue;
            }
        ?>
        <li>
            <strong>From Seller ID <?= $message['user_id'] ?>:</strong>
            <?= $message['message'] ?>
        </li>
    <?php endforeach; ?>
</ul>          
